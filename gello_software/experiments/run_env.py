...
def main(args):
    ...
    if (action - joints > 0.5).any():
        ...
        max_difference = np.max(np.abs(action - joints))
        print("Maximum difference:", max_difference)
        max_index = np.argmax(np.abs(action - joints))
        print("Maximum difference joint:", max_index)
    ...
    while True:
        ...
        joints = obs["joint_positions"]
        action = agent.act(obs)
        if(abs(joints-action)>0.9).any():
            raise RuntimeError("Action is too big")

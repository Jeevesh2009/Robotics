...
def inverse_kinematics(pos):
    ...
    cos_theta2 = np.clip(cos_theta2, -1.0, 1.0)  # Clip to valid range
    theta2 = math.acos(cos_theta2)
    a=L2*math.sin(theta2)
    b=L1+L2*math.cos(theta2)
    theta1 = math.atan2(y,x)-math.atan2(a,b)

    return [theta1+pos_to_rad(1024), theta2+pos_to_rad(2048)]
...
def update_parameters():
    ...
    pos_d = [float(pos_var[i].get())for i in range(2)]
    x=pos_d[0]
    y=pos_d[1]
    if((x<0 and y<0) or (x>0 and y<0)):
        print("The coords are out of range")
        raise(RuntimeWarning)
    if(0.15<math.sqrt((x**2)+(y**2))<0.2):
        print("The coords are out of range")
        raise(RuntimeWarning)
    theta_d=inverse_kinematics(pos_d)
...
pos_var = [tk.StringVar(value=str(pos_d[i])) for i in range(2)]
...
tk.Label(root, text="x").grid(row=4, column=0)
tk.Entry(root, textvariable=pos_var[0]).grid(row=4, column=1)

tk.Label(root, text="y").grid(row=4, column=2)
tk.Entry(root, textvariable=pos_var[1]).grid(row=4, column=3)
...
theta_d_lines = [axs[0].plot([], [], 'r--', label=f'theta_d {1}')[0] , axs[0].plot([], [], 'g--', label=f'theta_d {2}')[0]]
axs[0].legend()
